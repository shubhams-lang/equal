/**
 * STICKER_PACKS
 * id: Unique identifier for the pack
 * name: Display name of the pack
 * stickers: Array of sticker objects { id, name, url }
 */

export const STICKER_PACKS = [
  {
    id: "essentials_pack",
    name: "Essentials",
    stickers: [
      {
        id: "smile_face",
        name: "happy smile face yellow",
        // Using a Base64 string ensures this sticker ALWAYS loads
        url: "data:image/png;base64,"
      },
      {
        id: "heart_red",
        name: "love heart red like",
        url: "https://cdn-icons-png.flaticon.com/512/833/833472.png" // Example external URL
      },
      {
        id: "fire_lit",
        name: "fire lit hot trending",
        url: "https://cdn-icons-png.flaticon.com/512/785/785116.png"
      }
    ]
  },
  {
    id: "memes_pack",
    name: "Memes",
    stickers: [
      {
        id: "doge_meme",
        name: "doge wow much meme dog",
        url: "https://cdn-icons-png.flaticon.com/512/1000/1000946.png"
      },
      {
        id: "cat_vibe",
        name: "cat vibing head bob",
        url: "https://cdn-icons-png.flaticon.com/512/1000/1000947.png"
      }
    ]
  }
];

/**
 * Helper function to find a sticker by ID if needed elsewhere
 */
export const getStickerById = (id) => {
  return STICKER_PACKS.flatMap(pack => pack.stickers).find(s => s.id === id);
};